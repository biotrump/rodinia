#!/bin/bash
#export TOOLCHAIN_ROOT=${HOME}/gcc-linaro-4.9-2015.05-x86_64_aarch64-linux-gnu
#export TOOLCHAIN_ROOT=${HOME}/gcc-linaro-4.9-2015.05-x86_64_arm-linux-gnueabihf
#GCC_COMPILER_VERSION="4.9"

#opencl 1.1 for rk3288, T76x
export OPENCL_VER="1.1"
export OPENCL_INC="${HOME}/openCL/include/${OPENCL_VER}"
#mali user space driver
export OPENCL_LIB="${HOME}/openCL/rock-chips/rk3288/userspace/mali-t76x_r5p0-06rel0_linux_1+fbdev/fbdev"
...
export kmeans_SRC=`pwd`
export kmeans_BUILD=`pwd`/build

if [ -d "$kmeans_BUILD/$APP_ABI" ]; then
	if [ -n "$clean_build" ]; then
		rm -rf $kmeans_BUILD/$APP_ABI/*
	fi
else
	mkdir -p $kmeans_BUILD/$APP_ABI
fi
echo $kmeans_BUILD/$APP_ABI

pushd ${kmeans_BUILD}/$APP_ABI
export ARM_ABI=${APP_ABI}

case $APP_ABI in
	armeabi)
		cmake -DCMAKE_TOOLCHAIN_FILE=${kmeans_SRC}/arm-gnueabi.toolchain.cmake \
